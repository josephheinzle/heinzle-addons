...
local EXPECTED_PLATFORM = 0  -- 0 = XBNA, 3 = XBEU, 1 = PSNA, 4 = PSEU

--[[ QR Code window tracking (LibQRCode no longer used)
...
local function getFormattedAvgPrice(itemLink)
    local result, _ = getAvgPrice(itemLink)
    if result == TSCPriceDataAPI.LOADING then
...
local function getFormattedPriceRange(itemLink)
    ...
    if not itemData or itemData == TSCPriceDataAPI.LOADING or not itemData.commonMin or not itemData.commonMax then
        return nil
    end
...
local TSC_PRICE_COLOR_LEGACY = "|c888888"  -- muted gray
...
local function initializeSavedVars()
    ...
    TSC.savedVars = ZO_SavedVars:NewAccountWide(
        "TSCPriceFetcherDataV2", SAVED_VARS_VERSION, nil, TSC.default)
...
local function setupSettingsMenu()
    ...
    local whatsNewButton = {
        type = LHAS.ST_BUTTON,
        label = "What's New",
        tooltip = [[v200: One addon for all console servers! The addon now lazy loads sales data. It also shows price variations by Item name and Exact match (quality and trait matching).
...
Scan the QR code to view full update details]],
    ...
    local whatsNewButton = {
        type = LHAS.ST_BUTTON,
        label = "What's New",
        tooltip = [[v203: One addon for all console servers! The addon now lazy loads sales data. It also shows price variations by Item name and Exact match (quality and trait matching).

Opens the update page in your browser.]],
        buttonText = "View Update Info",
        clickHandler = function(control, button)
            RequestOpenUnsafeURL("https://tamrielsavings.com/updates")
        end,
    }
...
local function checkServerCompatibility()
    ...
    -- Treat PC NA (2) as XB NA (0) for compatibility; we use XBNA data for PCNA
    if currentPlatform == 2 then currentPlatform = 0 end
